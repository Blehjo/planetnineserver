// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Planetnineserver.Data;

#nullable disable

namespace K.Migrations
{
    [DbContext(typeof(Planetnineservercontext))]
    [Migration("20230613050912_InitialCreate")]
    partial class InitialCreate
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "7.0.2");

            modelBuilder.Entity("Planetnineserver.Models.ArtificialIntelligence", b =>
                {
                    b.Property<int>("ArtificialIntelligenceId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("ImageLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.Property<string>("Role")
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("ArtificialIntelligenceId");

                    b.HasIndex("UserId");

                    b.ToTable("ArtificialIntelligences");
                });

            modelBuilder.Entity("Planetnineserver.Models.Chat", b =>
                {
                    b.Property<int>("ChatId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("ArtificialId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("ArtificialIntelligenceId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("ChatId");

                    b.HasIndex("ArtificialIntelligenceId");

                    b.HasIndex("UserId");

                    b.ToTable("Chats");
                });

            modelBuilder.Entity("Planetnineserver.Models.ChatComment", b =>
                {
                    b.Property<int>("ChatCommentId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int?>("ArtificialIntelligenceId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ChatId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("ChatValue")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MediaLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("Type")
                        .HasColumnType("TEXT");

                    b.HasKey("ChatCommentId");

                    b.HasIndex("ArtificialIntelligenceId");

                    b.HasIndex("ChatId");

                    b.ToTable("ChatComments");
                });

            modelBuilder.Entity("Planetnineserver.Models.Comment", b =>
                {
                    b.Property<int>("CommentId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int?>("ChatId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("CommentValue")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MediaLink")
                        .HasColumnType("TEXT");

                    b.Property<int>("PostId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("CommentId");

                    b.HasIndex("ChatId");

                    b.HasIndex("PostId");

                    b.HasIndex("UserId");

                    b.ToTable("Comment");
                });

            modelBuilder.Entity("Planetnineserver.Models.Favorite", b =>
                {
                    b.Property<int>("FavoriteId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int?>("ChatCommentId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("ChatId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("CommentId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ContentId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("ContentType")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<int?>("MessageCommentId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("MoonCommentId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("MoonId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("PlanetCommentId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("PlanetId")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("PostId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("FavoriteId");

                    b.HasIndex("ChatCommentId");

                    b.HasIndex("ChatId");

                    b.HasIndex("CommentId");

                    b.HasIndex("MessageCommentId");

                    b.HasIndex("MoonCommentId");

                    b.HasIndex("MoonId");

                    b.HasIndex("PlanetCommentId");

                    b.HasIndex("PlanetId");

                    b.HasIndex("PostId");

                    b.HasIndex("UserId");

                    b.ToTable("Favorite");
                });

            modelBuilder.Entity("Planetnineserver.Models.Follower", b =>
                {
                    b.Property<int>("FollowerId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<int>("FollowerUser")
                        .HasColumnType("INTEGER");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("FollowerId");

                    b.HasIndex("UserId");

                    b.ToTable("Follower");
                });

            modelBuilder.Entity("Planetnineserver.Models.Message", b =>
                {
                    b.Property<int>("MessageId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MessageValue")
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("MessageId");

                    b.HasIndex("UserId");

                    b.ToTable("Message");
                });

            modelBuilder.Entity("Planetnineserver.Models.MessageComment", b =>
                {
                    b.Property<int>("MessageCommentId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MediaLink")
                        .HasColumnType("TEXT");

                    b.Property<int>("MessageId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("MessageValue")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("MessageCommentId");

                    b.HasIndex("MessageId");

                    b.HasIndex("UserId");

                    b.ToTable("MessageComment");
                });

            modelBuilder.Entity("Planetnineserver.Models.Moon", b =>
                {
                    b.Property<int>("MoonId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("Aphelion")
                        .HasColumnType("TEXT");

                    b.Property<string>("Brief")
                        .HasColumnType("TEXT");

                    b.Property<string>("Description")
                        .HasColumnType("TEXT");

                    b.Property<string>("Gravity")
                        .HasColumnType("TEXT");

                    b.Property<string>("ImageLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("ModelLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("MoonMass")
                        .HasColumnType("TEXT");

                    b.Property<string>("MoonName")
                        .HasColumnType("TEXT");

                    b.Property<string>("Perihelion")
                        .HasColumnType("TEXT");

                    b.Property<int>("PlanetId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Temperature")
                        .HasColumnType("TEXT");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("MoonId");

                    b.HasIndex("PlanetId");

                    b.HasIndex("UserId");

                    b.ToTable("Moon");

                    b.HasData(
                        new
                        {
                            MoonId = 1,
                            Aphelion = "405500",
                            Brief = "The fifth largest moon in the solar system, Earth's moon is the only place beyond Earth where humans have set foot.",
                            Description = "Earth's Moon is the only place beyond Earth where humans have set foot. The brightest and largest object in our night sky, the Moon makes Earth a more livable planet by moderating our home planet's wobble on its axis, leading to a relatively stable climate. It also causes tides, creating a rhythm that has guided humans for thousands of years. The Moon was likely formed after a Mars-sized body collided with Earth.Earth's Moon is the fifth largest of the 200+ moons orbiting planets in our solar system.",
                            Gravity = "1.62",
                            ImageLink = "https://solarsystem.nasa.gov/rails/active_--12f04d32265fa546e880584dbd51d437d716afd0/lro_nearside_800.jpg",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2366",
                            MoonMass = "7.346",
                            MoonName = "Moon",
                            Perihelion = "363300",
                            PlanetId = 4,
                            Temperature = "3.344",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 2,
                            Aphelion = "9518",
                            Brief = "Phobos, gouged and nearly shattered by a giant impact crater and beaten by thousands of meteorite impacts, is on a collision course with Mars.",
                            Description = "New modeling indicates that the grooves on Mars’ moon Phobos could be produced by tidal forces – the mutual gravitational pull of the planet and the moon. Initially, scientists had thought the grooves were created by the massive impact that made Stickney crater (lower right). Phobos, gouged and nearly shattered by a giant impact crater and beaten by thousands of meteorite impacts, is on a collision course with Mars. Phobos is the larger of Mars' two moons and is 17 x 14 x 11 miles (27 by 22 by 18 kilometers) in diameter. It orbits Mars three times a day, and is so close to the planet's surface that in some locations on Mars it cannot always be seen. Phobos is nearing Mars at a rate of six feet (1.8 meters) every hundred years; at that rate, it will either crash into Mars in 50 million years or break up into a ring. Its most prominent feature is the 6-mile (9.7 kilometer) crater Stickney, its impact causing streak patterns across the moon's surface. Stickney was seen by Mars Global Surveyor to be filled with fine dust, with evidence of boulders sliding down its sloped surface.",
                            Gravity = "0.0057",
                            ImageLink = "https://www.google.com/imgres?imgurl=https%3A%2F%2Fplanetary.s3.amazonaws.com%2Fweb%2Fassets%2Fpictures%2Fphobos_mro_globe_color_PSP_007769_9010_IRB_northup.jpg&tbnid=wZ4E43TOr9wfbM&vet=12ahUKEwjY4ouds7__AhVGNN4AHczKBfgQMygLegUIARD-AQ..i&imgrefurl=https%3A%2F%2Fwww.planetary.org%2Fspace-images%2Fcolor-view-of-phobos-from-mars-reconnaissance-orbiter&docid=4DjRBd-_uaBWIM&w=3804&h=3403&q=phobos%20moon&ved=2ahUKEwjY4ouds7__AhVGNN4AHczKBfgQMygLegUIARD-AQ",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2358",
                            MoonMass = "1.06",
                            MoonName = "Phobos",
                            Perihelion = "9234",
                            PlanetId = 5,
                            Temperature = "1.9",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 3,
                            Aphelion = "23471",
                            Brief = "Deimos is the smaller of Mars' two moons. Being only 9 by 7 by 6.8 miles in size (15 by 12 by 11 kilometers), Deimos whirls around Mars every 30 hours.",
                            Description = "Like Phobos, Deimos is a small and lumpy, heavily cratered object. Its craters are generally smaller than 1.6 miles (2.5 kilometers) in diameter, however, and it lacks the grooves and ridges seen on Phobos. Typically when a meteorite hits a surface, surface material is thrown up and out of the resulting crater. The material usually falls back to the surface surrounding the crater. However, these ejecta deposits are not seen on Deimos, perhaps because the moon's gravity is so low that the ejecta escaped to space. Material does appear to have moved down slopes. Deimos also has a thick regolith, perhaps as deep as 328 feet (100 meters), formed as meteorites pulverized the surface. Deimos is a dark body that appears to be composed of C-type surface materials, similar to that of asteroids found in the outer asteroid belt.",
                            Gravity = "0.003",
                            ImageLink = "https://www.google.com/imgres?imgurl=https%3A%2F%2Fsolarsystem.nasa.gov%2Fsystem%2Fstellar_items%2Fimage_files%2F8_feature_1600x900_deimos_2.jpg&tbnid=S3YDeJG6RQSdsM&vet=12ahUKEwi59ubqs7__AhWnwMkDHYSkCSIQMygBegUIARDoAQ..i&imgrefurl=https%3A%2F%2Fsolarsystem.nasa.gov%2Fmoons%2Fmars-moons%2Fdeimos%2Fin-depth%2F&docid=UmhTmy3VK84G6M&w=1600&h=900&q=deimos%20moon&ved=2ahUKEwi59ubqs7__AhWnwMkDHYSkCSIQMygBegUIARDoAQ",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2434",
                            MoonMass = "1.4762",
                            MoonName = "Deimos",
                            Perihelion = "23456",
                            PlanetId = 5,
                            Temperature = "1.75",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 4,
                            Aphelion = "423529",
                            Brief = "Io is the most volcanically active body in the solar system.",
                            Description = "Jupiter's moon Io is the most volcanically active world in the Solar System, with hundreds of volcanoes, some erupting lava fountains dozens of miles (or kilometers) high. Io is caught in a tug-of-war between Jupiter's massive gravity and the smaller but precisely timed pulls from two neighboring moons that orbit farther from Jupiter—Europa and Ganymede.",
                            Gravity = "1.79",
                            ImageLink = "",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2379",
                            MoonMass = "8.932",
                            MoonName = "Io",
                            Perihelion = "420071",
                            PlanetId = 6,
                            Temperature = "3.53",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 5,
                            Aphelion = "0",
                            Brief = "Europa may be the most promising place in our solar system to find present-day environments suitable for some form of life beyond Earth.",
                            Description = "Scientists are almost certain that hidden beneath the icy surface of Europa is a salty-water ocean thought to contain twice as much water as Earth’s oceans combined. And like Earth, Europa is thought to also contain a rocky mantle and iron core. There is very strong evidence suggesting Europa's ocean is in contact with rock.​ This is important because life as we know it requires three key basic ingredients: liquid water, an energy source, and organic compounds to use as the building blocks for biological processes.",
                            Gravity = "1.31",
                            ImageLink = "",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2388",
                            MoonMass = "4.8",
                            MoonName = "Europa",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "3.01",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 6,
                            Aphelion = "0",
                            Brief = "Ganymede is the largest satellite in our solar system. It is larger than Mercury and Pluto, and three-quarters the size of Mars.",
                            Description = "Jupiter’s icy moon Ganymede is the largest moon in our solar system, even bigger than the planet Mercury, and the dwarf planet Pluto. There’s strong evidence that Ganymede has an underground saltwater ocean that may hold more water than all the water on Earth's surface. It might even have ice and oceans stacked up in several layers like a club sandwich. Ganymede is the only moon known to have its own magnetic field – something typically found on planets like Earth. The magnetic field causes auroras, or bright ribbons of glowing gas, that circle the moon’s poles.",
                            Gravity = "1.428",
                            ImageLink = "",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2385",
                            MoonMass = "1.4819",
                            MoonName = "Ganymede",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1.94",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 7,
                            Aphelion = "0",
                            Brief = "Callisto is the most heavily cratered object in our solar system.",
                            Description = "Callisto is Jupiter’s second largest moon and the third largest moon in our solar system. Its surface is the most heavily cratered of any object in our solar system. Images of Callisto captured by passing spacecraft show bright white spots standing out against darker regions. Scientists think the bright areas are mostly ice and the darker patches are areas where the ice has eroded. Once thought to be a dead, inactive rocky body, data gathered by the Galileo spacecraft in the 1990s indicate Callisto may have a salty ocean beneath its icy surface. More recent research reveals that this ocean may be located deeper beneath the surface than previously thought, or may not exist at all. If an ocean is present, it’s possible the ocean is interacting with rock on Callisto, creating a potential habitat for life.",
                            Gravity = "1.235",
                            ImageLink = "",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2402",
                            MoonMass = "1.0759",
                            MoonName = "Callisto",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1.83",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 8,
                            Aphelion = "182840",
                            Brief = "Amalthea is the reddest object in the solar system and it appears to give out more heat than it receives from the sun. This may be because, as it orbits within Jupiter's powerful magnetic field, electric currents are included in the moon's core. Alternatively, the heat could be from tidal stresses.",
                            Description = "Orbiting within Io's orbit, which is the innermost of the four largest moons of Jupiter (called the Galilean moons), are four smaller moons named Metis, Adrastea, Amalthea, and Thebe. All of the moons within this grouping are oddly shaped, lacking either the mass and/or fluidity of composition to pull themselves into a reasonably spherical shape. The Galileo spacecraft has revealed some surface features, including impact craters, hills, and valleys. Amalthea is the largest within this grouping with a mean radius of about 83.5 ± 2.4 km. Amalthea orbits 181,400 km from its parent planet, Jupiter. Amalthea takes 0.498 Earth days to complete one orbit.",
                            Gravity = "0.02",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7.5",
                            MoonName = "Amalthea",
                            Perihelion = "181150",
                            PlanetId = 6,
                            Temperature = "3.1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 9,
                            Aphelion = "13082000",
                            Brief = "Himalia is the fifth largest moon orbiting Jupiter. With a mean radius of 85 km assuming an albedo of 0.04), it's only about 5% the size of the fourth largest moon, Europa. But it's by far the largest member of the Himalia group, a family of Jovian satellites which have similar orbits and appearance, and are therefore thought to have a common origin.",
                            Description = "Himalia is the fifth largest moon orbiting Jupiter. With a mean radius of 85 km assuming an albedo of 0.04), it's only about 5% the size of the fourth largest moon, Europa. But it's by far the largest member of the Himalia group, a family of Jovian satellites which have similar orbits and appearance, and are therefore thought to have a common origin.",
                            Gravity = "0.062",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9.5",
                            MoonName = "Himalia",
                            Perihelion = "9782900",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 10,
                            Aphelion = "0",
                            Brief = "Elara is the eighth largest moon of Jupiter. With a mean radius of 26.7 miles (43 km) assuming an albedo of 0.04, it's only about 2 percent the size of Europa, the smallest of the four Galilean moons. But it's about half as big as Himalia, which makes it the second biggest in the Himalia group, a family of Jovian satellites which have similar orbits and appearance, and are therefore thought to have a common origin.",
                            Description = "Elara is the eighth largest moon of Jupiter. With a mean radius of 26.7 miles (43 km) assuming an albedo of 0.04, it's only about 2 percent the size of Europa, the smallest of the four Galilean moons. But it's about half as big as Himalia, which makes it the second biggest in the Himalia group, a family of Jovian satellites which have similar orbits and appearance, and are therefore thought to have a common origin.",
                            Gravity = "0.031",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8",
                            MoonName = "Elara",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 11,
                            Aphelion = "0",
                            Brief = "Pasiphae is the largest member of the Pasiphae group, a family of Jovian satellites which have similar orbits and are therefore thought to have a common origin. Pasiphae was probably an asteroid that was captured by Jupiter's gravity and then suffered a collision which broke off a number of pieces. Those pieces became at least some, and perhaps all, of the other moons commonly categorized as members of the Pasiphae group.",
                            Description = "Pasiphae is the largest member of the Pasiphae group, a family of Jovian satellites which have similar orbits and are therefore thought to have a common origin. Pasiphae was probably an asteroid that was captured by Jupiter's gravity and then suffered a collision which broke off a number of pieces. Those pieces became at least some, and perhaps all, of the other moons commonly categorized as members of the Pasiphae group.",
                            Gravity = "0.022",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3",
                            MoonName = "Pasiphae",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 12,
                            Aphelion = "0",
                            Brief = "Sinope is generally considered a member of the Pasiphae group, a family of Jovian satellites which have similar orbits and are therefore thought to have a common origin. However, there is some uncertainty about whether Sinope belongs in this group or not.",
                            Description = "Sinope is generally considered a member of the Pasiphae group, a family of Jovian satellites which have similar orbits and are therefore thought to have a common origin. However, there is some uncertainty about whether Sinope belongs in this group or not.",
                            Gravity = "0.014",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8",
                            MoonName = "Sinope",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 13,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.013",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8",
                            MoonName = "Lysithea",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 14,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.017",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Carme",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 15,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.01",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4",
                            MoonName = "Ananke",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 16,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0073",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6",
                            MoonName = "Leda",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 17,
                            Aphelion = "226000",
                            Brief = "",
                            Description = "",
                            Gravity = "0.02",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8",
                            MoonName = "Thebe",
                            Perihelion = "218000",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 18,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.002",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Adrastea",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 19,
                            Aphelion = "128026",
                            Brief = "",
                            Description = "",
                            Gravity = "0.005",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Metis",
                            Perihelion = "127974",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 20,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0031",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8.7",
                            MoonName = "Callirrhoe",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 21,
                            Aphelion = "8874300",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0029",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6.9",
                            MoonName = "Themisto",
                            Perihelion = "5909000",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 22,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.1",
                            MoonName = "Megaclite",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 23,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.6",
                            MoonName = "Taygete",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 24,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7.5",
                            MoonName = "Chaldene",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 25,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.2",
                            MoonName = "Harpalyke",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 26,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.9",
                            MoonName = "Kalyke",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 27,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.9",
                            MoonName = "Iocaste",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 28,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Erinome",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 29,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7.5",
                            MoonName = "Isonoe",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 30,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.3",
                            MoonName = "Praxidike",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 31,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0015",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Autonoe",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 32,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0015",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Thyone",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 33,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0015",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Hermippe",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 34,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0012",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Aitne",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 35,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0012",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Eurydome",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 36,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0012",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Euanthe",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 37,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Euporie",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 38,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.00081",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Orthosie",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 39,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.00081",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Sponde",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 40,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.00081",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Kale",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 41,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.00081",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Pasithee",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 42,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Hegemone",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 43,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Mneme",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 44,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Aoede",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 45,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Thelxinoe",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 46,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.0012",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.15",
                            MoonName = "Arche",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 47,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Kallichore",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 48,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Helike",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 49,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Carpo",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 50,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Eukelade",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 51,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Cyllene",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 52,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Kore",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 53,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Herse",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 54,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 2",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 55,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Eupheme",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 56,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 4",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 57,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Eirene",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 58,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 9",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 59,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 10",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 60,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "S/2003 J 12",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 61,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Philophrosyne",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 62,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 16",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 63,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 18",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 64,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 19",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 65,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.00081",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2003 J 23",
                            Perihelion = "0",
                            PlanetId = 6,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 66,
                            Aphelion = "189270",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.79",
                            MoonName = "Mimas",
                            Perihelion = "181770",
                            PlanetId = 7,
                            Temperature = "1.15",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 67,
                            Aphelion = "239066",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.08",
                            MoonName = "Enceladus",
                            Perihelion = "236830",
                            PlanetId = 7,
                            Temperature = "1.61",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 68,
                            Aphelion = "294648",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6.18",
                            MoonName = "Tethys",
                            Perihelion = "294589",
                            PlanetId = 7,
                            Temperature = "0.985",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 69,
                            Aphelion = "378260",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.095",
                            MoonName = "Dione",
                            Perihelion = "376580",
                            PlanetId = 7,
                            Temperature = "1.48",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 70,
                            Aphelion = "527597",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Rhea",
                            Perihelion = "526543",
                            PlanetId = 7,
                            Temperature = "1.24",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 71,
                            Aphelion = "1257060",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.3452",
                            MoonName = "Titan",
                            Perihelion = "1186680",
                            PlanetId = 7,
                            Temperature = "1.88",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 72,
                            Aphelion = "1535756",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5.6",
                            MoonName = "Hyperion",
                            Perihelion = "1466112",
                            PlanetId = 7,
                            Temperature = "0.55",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 73,
                            Aphelion = "3661612",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.805",
                            MoonName = "Iapetus",
                            Perihelion = "3460068",
                            PlanetId = 7,
                            Temperature = "1.09",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 74,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8.292",
                            MoonName = "Phoebe",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1.64",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 75,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.9",
                            MoonName = "Janus",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.63",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 76,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5.3",
                            MoonName = "Epimetheus",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.64",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 77,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "null",
                            MoonName = "Helene",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1.3",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 78,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Telesto",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 79,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6.5",
                            MoonName = "Calypso",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 80,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7",
                            MoonName = "Atlas",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.5",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 81,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.6",
                            MoonName = "Prometheus",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.48",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 82,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.4",
                            MoonName = "Pandora",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.49",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 83,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.95",
                            MoonName = "Pan",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.42",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 84,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.97",
                            MoonName = "Ymir",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 85,
                            Aphelion = "23139965",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7.25",
                            MoonName = "Paaliaq",
                            Perihelion = "6908035",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 86,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Tarvos",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 87,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.18",
                            MoonName = "Ijiraq",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 88,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Suttungr",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 89,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.79",
                            MoonName = "Kiviuq",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 90,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Mundilfari",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 91,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.23",
                            MoonName = "Albiorix",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 92,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.5",
                            MoonName = "Skathi",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 93,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6.8",
                            MoonName = "Erriapus",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 94,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.35",
                            MoonName = "Siarnaq",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 95,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Thrymr",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 96,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Narvi",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 97,
                            Aphelion = "194459",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Methone",
                            Perihelion = "194421",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 98,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6",
                            MoonName = "Pallene",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 99,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Polydeuces",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 100,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Daphnis",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "0.34",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 101,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Aegir",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 102,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Bebhionn",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 103,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Bergelmir",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 104,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Bestla",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 105,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "Farbauti",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 106,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5",
                            MoonName = "Fenrir",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 107,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Fornjot",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 108,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Hati",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 109,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.5",
                            MoonName = "Hyrrokkin",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 110,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Kari",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 111,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Loge",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 112,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Skoll",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 113,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Surtur",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 114,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5",
                            MoonName = "Anthe",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 115,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Jarnsaxa",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 116,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "Greip",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 117,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Tarqeq",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 118,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Aegaeon",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 119,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2004 S 7",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 120,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9",
                            MoonName = "S/2004 S 12",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 121,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2004 S 13",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 122,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5",
                            MoonName = "S/2004 S 17",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 123,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "S/2006 S 1",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 124,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2006 S 3",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 125,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2007 S 2",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 126,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.5",
                            MoonName = "S/2007 S 3",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 127,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "S/2009 S 1",
                            Perihelion = "0",
                            PlanetId = 7,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 128,
                            Aphelion = "191130",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "12.9",
                            MoonName = "Ariel",
                            Perihelion = "190670",
                            PlanetId = 8,
                            Temperature = "1.59",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 129,
                            Aphelion = "267500",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "12.2",
                            MoonName = "Umbriel",
                            Perihelion = "265100",
                            PlanetId = 8,
                            Temperature = "1.46",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 130,
                            Aphelion = "436800",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "34.2",
                            MoonName = "Titania",
                            Perihelion = "435800",
                            PlanetId = 8,
                            Temperature = "1.66",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 131,
                            Aphelion = "584336",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "28.8",
                            MoonName = "Oberon",
                            Perihelion = "582702",
                            PlanetId = 8,
                            Temperature = "1.56",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 132,
                            Aphelion = "130041",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "6.6",
                            MoonName = "Miranda",
                            Perihelion = "129703",
                            PlanetId = 8,
                            Temperature = "1.2",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 133,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.5",
                            MoonName = "Cordelia",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 134,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5.4",
                            MoonName = "Ophelia",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 135,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "9.2",
                            MoonName = "Bianca",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 136,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.4",
                            MoonName = "Cressida",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 137,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.8",
                            MoonName = "Desdemona",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 138,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5.6",
                            MoonName = "Juliet",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 139,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.7",
                            MoonName = "Portia",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 140,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "0.25",
                            MoonName = "Rosalind",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 141,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4.9",
                            MoonName = "Belinda",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 142,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.9",
                            MoonName = "Puck",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 143,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.5",
                            MoonName = "Caliban",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 144,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.3",
                            MoonName = "Sycorax",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 145,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "8.5",
                            MoonName = "Prospero",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 146,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7.5",
                            MoonName = "Setebos",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 147,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.2",
                            MoonName = "Stephano",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 148,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.9",
                            MoonName = "Trinculo",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 149,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "7.2",
                            MoonName = "Francisco",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 150,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5.4",
                            MoonName = "Margaret",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 151,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5.4",
                            MoonName = "Ferdinand",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 152,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1.8",
                            MoonName = "Perdita",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 153,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Mab",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 154,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3.8",
                            MoonName = "Cupid",
                            Perihelion = "0",
                            PlanetId = 8,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 155,
                            Aphelion = "354765",
                            Brief = "",
                            Description = "",
                            Gravity = "0.78",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2.14",
                            MoonName = "Triton",
                            Perihelion = "354753",
                            PlanetId = 9,
                            Temperature = "2.05",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 156,
                            Aphelion = "9655000",
                            Brief = "",
                            Description = "",
                            Gravity = "0.071",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "3",
                            MoonName = "Nereid",
                            Perihelion = "1372000",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 157,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.01",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Naiad",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 158,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.013",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "4",
                            MoonName = "Thalassa",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 159,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.023",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Despina",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 160,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.03",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Galatea",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 161,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.034",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5",
                            MoonName = "Larissa",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 162,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.075",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "5",
                            MoonName = "Proteus",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 163,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.01",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Halimede",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 164,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "2",
                            MoonName = "Psamathe",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 165,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.01",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Sao",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 166,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0.01",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Laomedeia",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        },
                        new
                        {
                            MoonId = 167,
                            Aphelion = "0",
                            Brief = "",
                            Description = "",
                            Gravity = "0",
                            ImageLink = "",
                            ModelLink = "",
                            MoonMass = "1",
                            MoonName = "Neso",
                            Perihelion = "0",
                            PlanetId = 9,
                            Temperature = "1",
                            Type = "moon",
                            UserId = 1
                        });
                });

            modelBuilder.Entity("Planetnineserver.Models.MoonComment", b =>
                {
                    b.Property<int>("MoonCommentId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("CommentValue")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MediaLink")
                        .HasColumnType("TEXT");

                    b.Property<int>("MoonId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("MoonCommentId");

                    b.HasIndex("MoonId");

                    b.HasIndex("UserId");

                    b.ToTable("MoonComment");
                });

            modelBuilder.Entity("Planetnineserver.Models.Planet", b =>
                {
                    b.Property<int>("PlanetId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("Aphelion")
                        .HasColumnType("TEXT");

                    b.Property<string>("Brief")
                        .HasColumnType("TEXT");

                    b.Property<string>("Description")
                        .HasColumnType("TEXT");

                    b.Property<string>("Gravity")
                        .HasColumnType("TEXT");

                    b.Property<string>("ImageLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("ModelLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("Perihelion")
                        .HasColumnType("TEXT");

                    b.Property<string>("PlanetMass")
                        .HasColumnType("TEXT");

                    b.Property<string>("PlanetName")
                        .HasColumnType("TEXT");

                    b.Property<string>("Temperature")
                        .HasColumnType("TEXT");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int?>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("PlanetId");

                    b.HasIndex("UserId");

                    b.ToTable("Planet");

                    b.HasData(
                        new
                        {
                            PlanetId = 2,
                            Aphelion = "69816900",
                            Brief = "From the surface of Mercury, the Sun would appear more than three times as large as it does when viewed from Earth, and the sunlight would be as much as 11 times brighter.",
                            Description = "The smallest planet in our solar system and nearest to the Sun, Mercury is only slightly larger than Earth's Moon. From the surface of Mercury, the Sun would appear more than three times as large as it does when viewed from Earth, and the sunlight would be as much as seven times brighter. Despite its proximity to the Sun, Mercury is not the hottest planet in our solar system – that title belongs to nearby Venus, thanks to its dense atmosphere. Because of Mercury's elliptical – egg-shaped – orbit, and sluggish rotation, the Sun appears to rise briefly, set, and rise again from some parts of the planet's surface. The same thing happens in reverse at sunset.",
                            Gravity = "3.7",
                            ImageLink = "https://solarsystem.nasa.gov/rails/active_storage/blobs/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBc0VRIiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--b43e79a5b43d2063c4866ccff66bede5faff17b8/mercury_th.jpg",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2369",
                            Perihelion = "46001200",
                            PlanetMass = "3.30114",
                            PlanetName = "Mercury",
                            Temperature = "5.4291",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 3,
                            Aphelion = "108939000",
                            Brief = "Similar in structure and size to Earth, Venus's thick atmosphere traps heat in a runaway greenhouse effect, making it the hottest planet in our solar system.",
                            Description = "Venus is the second planet from the Sun and is Earth’s closest planetary neighbor. It’s one of the four inner, terrestrial (or rocky) planets, and it’s often called Earth’s twin because it’s similar in size and density. These are not identical twins, however – there are radical differences between the two worlds. Venus has a thick, toxic atmosphere filled with carbon dioxide and it’s perpetually shrouded in thick, yellowish clouds of sulfuric acid that trap heat, causing a runaway greenhouse effect. It’s the hottest planet in our solar system, even though Mercury is closer to the Sun. Surface temperatures on Venus are about 900 degrees Fahrenheit (475 degrees Celsius) – hot enough to melt lead. The surface is a rusty color and it’s peppered with intensely crunched mountains and thousands of large volcanoes. Scientists think it’s possible some volcanoes are still active. Another big difference from Earth – Venus rotates on its axis backward, compared to most of the other planets in the solar system. This means that, on Venus, the Sun rises in the west and sets in the east, opposite to what we experience on Earth. (It’s not the only planet in our solar system with such an oddball rotation – Uranus spins on its side.)",
                            Gravity = "8.87",
                            ImageLink = "https://solarsystem.nasa.gov/rails/active_storage/blobs/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBajl5IiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--1771be6bd46710c30aa93cd6c3ababe23ad52681/480x320_venus.png?disposition=inline",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2343",
                            Perihelion = "107477000",
                            PlanetMass = "4.86747",
                            PlanetName = "Venus",
                            Temperature = "5.243",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 4,
                            Aphelion = "152100000",
                            Brief = "Earth—our home planet—is the only place we know of so far that’s inhabited by living things. It's also the only planet in our solar system with liquid water on the surface.",
                            Description = "Our home planet is the third planet from the Sun, and the only place we know of so far that’s inhabited by living things. While Earth is only the fifth largest planet in the solar system, it is the only world in our solar system with liquid water on the surface. Just slightly larger than nearby Venus, Earth is the biggest of the four planets closest to the Sun, all of which are made of rock and metal. Earth is the only planet in the Solar System whose English name does not come from Greek or Roman mythology. The name was taken from Old English and Germanic. It simply means the ground. There are, of course, many names for our planet in the thousands of languages spoken by the people of the third planet from the Sun.",
                            Gravity = "9.8",
                            ImageLink = "https://upload.wikimedia.org/wikipedia/commons/thumb/6/60/Earth_from_Space.jpg/2048px-Earth_from_Space.jpg",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2393",
                            Perihelion = "147095000",
                            PlanetMass = "5.97237",
                            PlanetName = "Earth",
                            Temperature = "5.5136",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 5,
                            Aphelion = "249200000",
                            Brief = "Mars is a dusty, cold, desert world with a very thin atmosphere. There is strong evidence Mars was – billions of years ago – wetter and warmer, with a thicker atmosphere.",
                            Description = "​Mars is the fourth planet from the Sun – a dusty, cold, desert world with a very thin atmosphere. Mars is also a dynamic planet with seasons, polar ice caps, canyons, extinct volcanoes, and evidence that it was even more active in the past. Mars is one of the most explored bodies in our solar system, and it's the only planet where we've sent rovers to roam the alien landscape. NASA currently has two rovers (Curiosity and Perseverance), one lander (InSight), and one helicopter (Ingenuity) exploring the surface of Mars.",
                            Gravity = "3.7",
                            ImageLink = "https://solarsystem.nasa.gov/system/stellar_items/image_files/6_mars.jpg",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2372",
                            Perihelion = "206700000",
                            PlanetMass = "3.71",
                            PlanetName = "Mars",
                            Temperature = "3.9341",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 6,
                            Aphelion = "816620000",
                            Brief = "Jupiter is more than twice as massive than the other planets of our solar system combined. The giant planet's Great Red Spot is a centuries-old storm bigger than Earth.",
                            Description = "Jupiter has a long history of surprising scientists – all the way back to 1610 when Galileo Galilei found the first moons beyond Earth. That discovery changed the way we see the universe. Jupiter's familiar stripes and swirls are actually cold, windy clouds of ammonia and water, floating in an atmosphere of hydrogen and helium. Jupiter’s iconic Great Red Spot is a giant storm bigger than Earth that has raged for hundreds of years",
                            Gravity = "24.79",
                            ImageLink = "https://solarsystem.nasa.gov/rails/active_storage/blobs/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBdGxyIiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--229ba26c079b7122001319d5dcffec49c8d4e0cb/STSCI-H-p1936a-f-640.jpg?disposition=attachment",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2375",
                            Perihelion = "740379835",
                            PlanetMass = "1.89819",
                            PlanetName = "Jupiter",
                            Temperature = "1.3262",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 7,
                            Aphelion = "1503509229",
                            Brief = "Adorned with a dazzling, complex system of icy rings, Saturn is unique in our solar system. The other giant planets have rings, but none are as spectacular as Saturn's.",
                            Description = "Saturn is the sixth planet from the Sun and the second-largest planet in our solar system. Adorned with thousands of beautiful ringlets, Saturn is unique among the planets. It is not the only planet to have rings – made of chunks of ice and rock – but none are as spectacular or as complicated as Saturn's. Like fellow gas giant Jupiter, Saturn is a massive ball made mostly of hydrogen and helium.",
                            Gravity = "10.44",
                            ImageLink = "https://solarsystem.nasa.gov/system/stellar_items/image_files/38_saturn_1600x900.jpg",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2355",
                            Perihelion = "1349823615",
                            PlanetMass = "5.68336",
                            PlanetName = "Saturn",
                            Temperature = "0.6871",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 8,
                            Aphelion = "3006318143",
                            Brief = "Uranus—seventh planet from the Sun—rotates at a nearly 90-degree angle from the plane of its orbit. This unique tilt makes Uranus appear to spin on its side.",
                            Description = "Uranus is the seventh planet from the Sun, and has the third-largest diameter in our solar system. It was the first planet found with the aid of a telescope, Uranus was discovered in 1781 by astronomer William Herschel, although he originally thought it was either a comet or a star. It was two years later that the object was universally accepted as a new planet, in part because of observations by astronomer Johann Elert Bode. Herschel tried unsuccessfully to name his discovery Georgium Sidus after King George III. Instead, the scientific community accepted Bode's suggestion to name it Uranus, the Greek god of the sky, as suggested by Bode.​",
                            Gravity = "8.87",
                            ImageLink = "https://images.english.elpais.com/resizer/BH_KvY_lAzwSrpp8v7D55nGax8A=/1960x1470/filters:focal(2464x2210:2474x2220)/cloudfront-eu-central-1.images.arcpublishing.com/prisa/WQ773ELGFRDOHJ46MOLRXKJ7AY.jpg",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2344&",
                            Perihelion = "2734998229",
                            PlanetMass = "8.68127",
                            PlanetName = "Uranus",
                            Temperature = "1.27",
                            Type = "planet",
                            UserId = 1
                        },
                        new
                        {
                            PlanetId = 9,
                            Aphelion = "4537039826",
                            Brief = "Neptune—the eighth and most distant major planet orbiting our Sun—is dark, cold and whipped by supersonic winds. It was the first planet located through mathematical calculations.",
                            Description = "Dark, cold, and whipped by supersonic winds, ice giant Neptune is the eighth and most distant planet in our solar system. More than 30 times as far from the Sun as Earth, Neptune is the only planet in our solar system not visible to the naked eye and the first predicted by mathematics before its discovery. In 2011 Neptune completed its first 165-year orbit since its discovery in 1846. NASA's Voyager 2 is the only spacecraft to have visited Neptune up close. It flew past in 1989 on its way out of the solar system.",
                            Gravity = "11.15",
                            ImageLink = "https://solarsystem.nasa.gov/rails/active_storage/blobs/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBbkp5IiwiZXhwIjpudWxsLCJwdXIiOiJibG9iX2lkIn19--cb7cb2868fd8b4892788893961c20975baffeb52/neptune_480x320.jpg?disposition=inline",
                            ModelLink = "https://solarsystem.nasa.gov/gltf_embed/2364",
                            Perihelion = "4459753056",
                            PlanetMass = "1.02413",
                            PlanetName = "Neptune",
                            Temperature = "1.638",
                            Type = "planet",
                            UserId = 1
                        });
                });

            modelBuilder.Entity("Planetnineserver.Models.PlanetComment", b =>
                {
                    b.Property<int>("PlanetCommentId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("CommentValue")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MediaLink")
                        .HasColumnType("TEXT");

                    b.Property<int>("PlanetId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("PlanetCommentId");

                    b.HasIndex("PlanetId");

                    b.HasIndex("UserId");

                    b.ToTable("PlanetComment");
                });

            modelBuilder.Entity("Planetnineserver.Models.Post", b =>
                {
                    b.Property<int>("PostId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<string>("MediaLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("PostValue")
                        .HasColumnType("TEXT");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("UserId")
                        .HasColumnType("INTEGER");

                    b.HasKey("PostId");

                    b.HasIndex("UserId");

                    b.ToTable("Post");
                });

            modelBuilder.Entity("Planetnineserver.Models.User", b =>
                {
                    b.Property<int>("UserId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("About")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateCreated")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("DateOfBirth")
                        .HasColumnType("TEXT");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("TEXT");

                    b.Property<string>("FirstName")
                        .HasColumnType("TEXT");

                    b.Property<string>("ImageLink")
                        .HasColumnType("TEXT");

                    b.Property<string>("LastName")
                        .HasColumnType("TEXT");

                    b.Property<string>("Password")
                        .HasColumnType("TEXT");

                    b.Property<string>("Username")
                        .HasColumnType("TEXT");

                    b.HasKey("UserId");

                    b.ToTable("User");

                    b.HasData(
                        new
                        {
                            UserId = 1,
                            About = "cyberpunk",
                            DateCreated = new DateTime(2023, 6, 13, 5, 9, 12, 58, DateTimeKind.Utc).AddTicks(9310),
                            DateOfBirth = new DateTime(1, 1, 1, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            EmailAddress = "spacemarauder@email.com",
                            FirstName = "malorian",
                            LastName = "major",
                            Password = "Password1",
                            Username = "spacemarauder"
                        });
                });

            modelBuilder.Entity("Planetnineserver.Models.ArtificialIntelligence", b =>
                {
                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Chat", b =>
                {
                    b.HasOne("Planetnineserver.Models.ArtificialIntelligence", "ArtificialIntelligence")
                        .WithMany("Chats")
                        .HasForeignKey("ArtificialIntelligenceId");

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ArtificialIntelligence");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.ChatComment", b =>
                {
                    b.HasOne("Planetnineserver.Models.ArtificialIntelligence", null)
                        .WithMany("ChatComments")
                        .HasForeignKey("ArtificialIntelligenceId");

                    b.HasOne("Planetnineserver.Models.Chat", "Chat")
                        .WithMany("ChatComments")
                        .HasForeignKey("ChatId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Chat");
                });

            modelBuilder.Entity("Planetnineserver.Models.Comment", b =>
                {
                    b.HasOne("Planetnineserver.Models.Chat", null)
                        .WithMany("Comments")
                        .HasForeignKey("ChatId");

                    b.HasOne("Planetnineserver.Models.Post", "Post")
                        .WithMany("Comments")
                        .HasForeignKey("PostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Post");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Favorite", b =>
                {
                    b.HasOne("Planetnineserver.Models.ChatComment", null)
                        .WithMany("Favorites")
                        .HasForeignKey("ChatCommentId");

                    b.HasOne("Planetnineserver.Models.Chat", null)
                        .WithMany("Favorites")
                        .HasForeignKey("ChatId");

                    b.HasOne("Planetnineserver.Models.Comment", null)
                        .WithMany("Favorites")
                        .HasForeignKey("CommentId");

                    b.HasOne("Planetnineserver.Models.MessageComment", null)
                        .WithMany("Favorites")
                        .HasForeignKey("MessageCommentId");

                    b.HasOne("Planetnineserver.Models.MoonComment", null)
                        .WithMany("Favorites")
                        .HasForeignKey("MoonCommentId");

                    b.HasOne("Planetnineserver.Models.Moon", null)
                        .WithMany("Favorites")
                        .HasForeignKey("MoonId");

                    b.HasOne("Planetnineserver.Models.PlanetComment", null)
                        .WithMany("Favorites")
                        .HasForeignKey("PlanetCommentId");

                    b.HasOne("Planetnineserver.Models.Planet", null)
                        .WithMany("Favorites")
                        .HasForeignKey("PlanetId");

                    b.HasOne("Planetnineserver.Models.Post", null)
                        .WithMany("Favorites")
                        .HasForeignKey("PostId");

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany("Favorites")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Follower", b =>
                {
                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany("Followers")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Message", b =>
                {
                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.MessageComment", b =>
                {
                    b.HasOne("Planetnineserver.Models.Message", "Message")
                        .WithMany("MessageComments")
                        .HasForeignKey("MessageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Message");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Moon", b =>
                {
                    b.HasOne("Planetnineserver.Models.Planet", "Planet")
                        .WithMany("Moons")
                        .HasForeignKey("PlanetId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany("Moons")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Planet");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.MoonComment", b =>
                {
                    b.HasOne("Planetnineserver.Models.Moon", "Moon")
                        .WithMany("MoonComment")
                        .HasForeignKey("MoonId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Moon");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Planet", b =>
                {
                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany("Planets")
                        .HasForeignKey("UserId");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.PlanetComment", b =>
                {
                    b.HasOne("Planetnineserver.Models.Planet", "Planet")
                        .WithMany("PlanetComments")
                        .HasForeignKey("PlanetId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Planet");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.Post", b =>
                {
                    b.HasOne("Planetnineserver.Models.User", "User")
                        .WithMany("Posts")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("Planetnineserver.Models.ArtificialIntelligence", b =>
                {
                    b.Navigation("ChatComments");

                    b.Navigation("Chats");
                });

            modelBuilder.Entity("Planetnineserver.Models.Chat", b =>
                {
                    b.Navigation("ChatComments");

                    b.Navigation("Comments");

                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.ChatComment", b =>
                {
                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.Comment", b =>
                {
                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.Message", b =>
                {
                    b.Navigation("MessageComments");
                });

            modelBuilder.Entity("Planetnineserver.Models.MessageComment", b =>
                {
                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.Moon", b =>
                {
                    b.Navigation("Favorites");

                    b.Navigation("MoonComment");
                });

            modelBuilder.Entity("Planetnineserver.Models.MoonComment", b =>
                {
                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.Planet", b =>
                {
                    b.Navigation("Favorites");

                    b.Navigation("Moons");

                    b.Navigation("PlanetComments");
                });

            modelBuilder.Entity("Planetnineserver.Models.PlanetComment", b =>
                {
                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.Post", b =>
                {
                    b.Navigation("Comments");

                    b.Navigation("Favorites");
                });

            modelBuilder.Entity("Planetnineserver.Models.User", b =>
                {
                    b.Navigation("Favorites");

                    b.Navigation("Followers");

                    b.Navigation("Moons");

                    b.Navigation("Planets");

                    b.Navigation("Posts");
                });
#pragma warning restore 612, 618
        }
    }
}
